...
public class WindowController {
    ...
    public void openSelectWindow(WindowEnum window){
        switch(window){
            case WindowEnum.MAIN -> openWindow(mainWindow);
            case WindowEnum.SETTING -> openWindow(settingsWindow);
            case WindowEnum.GOOD_SELECT -> openWindow(goodSelectWindow);
        }
    }

    public void openSupervisor(WindowEnum windowEnum){
        switch(windowEnum){
            case WindowEnum.SUPPLIER_SUPERVISOR -> {
                for(SupplierSupervisorWindow window : supplierSupervisorWindow){
                    if (!window.isVisible()){
                        openWindow(window);
                        return;
            ...
            case WindowEnum.CONSUMER_SUPERVISOR -> {
                for(ConsumerSupervisorWindow window : consumerSupervisorWindow){
                    if (!window.isVisible()){
                        openWindow(window);
                        return;
            ...
            case WindowEnum.GOOD_SUPERVISOR -> {
                for(GoodSupervisorWindow window : goodSupervisorWindow){
                    if (!window.isVisible()){
                        openWindow(window);
                        return;
    ...
    public void closeSelectWindow(WindowEnum window){
        switch(window){
            case WindowEnum.MAIN -> closeWindow(mainWindow);
            case WindowEnum.SETTING -> closeWindow(settingsWindow);
            case WindowEnum.GOOD_SELECT -> closeWindow(goodSelectWindow);
        }
    }
