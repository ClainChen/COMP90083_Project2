...
public class ConsumerController {
    ...
    public void updateConsumer(GoodController goodController, SupplierController supplierController) {
        ...
        for (Consumer c: consumers){
            ...
            for (GoodSummarizer good: goodController.totalGoods){
                ...
                double competitive = goodController.getGoodCompetitive(good.id, last7SoldNum, supplierController);
                double buyProp = Tools.propBuy(good.id, good.basic, good.convenient, good.luxury,
                        avgPrice, last7SoldNum, interInventory.get(good.id), competitive, newC, good.type.equals(Enums.GoodType.BASIC), true);
    ...
    private void buyGood(Consumer c, GoodController goodController, SupplierController supplierController){
        ...
        for (int goodID: c.cart){
            ...
            for (int supplierID: good.ownedBy){
                ...
                double competitive = goodController.getGoodCompetitive(supplierGood.id, supplierGood.totalSoldNum, supplierController);
                double propBuyFromSupplier =
                        Tools.propBuy(supplierGood.id, supplierGood.basic, supplierGood.convenience, supplierGood.luxury,
                                supplierGood.price, supplierGood.getLast7SoldNum(), 1, competitive, c,
                                supplierGood.type.equals(Enums.GoodType.BASIC), false);
    ...
    public double getTotalDemand(Enums.GoodType type){
        ...
        for (Consumer c: consumers){
            switch (type){
                case BASIC -> demand += c.basicDemand;
                case CONVENIENCE -> demand += c.convenientDemand;
                case LUXURY -> demand += c.luxuryDemand;
            }
        }
        return demand;
    }
